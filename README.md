...
- Explore workspace with interactive regular-season scoring trends, signed
  ...
  season table, and within-season team comparisons and signed gap rankings for
  goals scored, goals allowed, and goal differential against xG.
...
| `NWSL_SYNC_TIMEOUT` | `2m` | Maximum duration of one ASA refresh and cache transaction. |
| `NWSL_QUALIFICATION_BUDGET` | `5s` | Maximum time for one qualification calculation batch. |
| `NWSL_SCENARIO_BUDGET` | `10m` | Maximum time for one clinching-scenario discovery batch; it runs independently after fixture sync and qualification. |
...
domain reference documentation and Go conventions for shared, HTTP, cache,
forecast, qualification, scenario, scheduler, and synchronization telemetry. Its
